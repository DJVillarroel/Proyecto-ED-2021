 ...
 * Implementación de la interfaz PriorityQueue con un Heap
 * @param <K>
 * @param <V>
 */
public class HeapPQueue<K, V> implements PriorityQueue<K, V> {
	...
	/**
	 * Chequea si la clave pasada por parametro es invalida o pertenece a la cola con prioridad
	 * @param key clave a chequear
	 * @return key clave chequeada
	 * @throws InvalidKeyException si la clave es invalida
	 */
	private K checkKey(K key) throws InvalidKeyException {
		if (key == null) throw new InvalidKeyException("error: la clave es nula");
		try {
